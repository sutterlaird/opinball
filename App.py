...
stop = StopButton(pinspotWorld, buttonList, controls.zero)
save = SaveShowButton(pinspotWorld)
load = LoadShowButton(pinspotWorld)
singlemulti = SingleMultiButton()
selectAll = SelectAllButton(buttonList, singlemulti)
deselectAll = DeselectAllButton(buttonList)
autotarget = AutoTargetButton(pinspotWorld, buttonList)
phone = PhoneControlButton(pinspotWorld, buttonList)
